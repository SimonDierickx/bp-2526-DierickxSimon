...
Those digital sources are evaluated using the CRAAP test (Currency, Relevanc e, Accuracy, Authority and Purpose).
...
Together they focus on the cost, licensing, range, reliability, easo of config and deployability.
...
As clean \gls{los} was expected to be an important factor a three-nodes topology was designed, consisting of two elevated fixed nodes (base and relay).
The third node was then a mobile node so testing could easily be done and compared to one antoher.
The reasoning, difficulites and setup is documented in Chapter~\ref{ch:poc}.
...
\begin{enumerate}
  ...
  node and the mobile node. This was sued to establish the maximal single-hop range of the network.
...
The \emph{Range Test} module was configured on the mobile node. This module communciates a test packet every 60 seconds from the moment the mobile node is booted.
...
are sent to the base node where a CSV file can be extracted. That way you can immediately tell the sequences missing, and by as a result the timestamps/locations of no connection.

On top of the Range Test module the \emph{traceroute} function was also used, this wasn't to determine the dead zones while driving like the range test, this is used on the stop locatioons to test the full path to the base node.
...
All those measurements were taken across multiple testing days, different locations, tiomes of day and weather conditions where possible (even though thsi juli/august months in belgium were extremely dry so no real weather difference could be measured).
This was done so that environmental, weather and topoligical factors where taken into account, and not just based on a single working snapshot.
...
For the fourth phase, the encryption sub-question, the main analsyiss is based on the official Meshtastic documentation.
As described in Section~\ref{sec:sota-security}, the firmware's security model is analyzied as well.
%TODO

\section{Phase 5: Comparative analysis}%
\label{sec:method-comparison}

In the fifth and final phase all the collected results are compared with the initial literature findigs.
Using the evaluation cirteria established during the first phase a scoring matrix will be built.
Based on literature and on the \gls{poc} Meshtastic, \gls{aprs}, \gls{dmr} and satellite messengers are compared. This can be found in Chapter~\ref{ch:conclusies}.