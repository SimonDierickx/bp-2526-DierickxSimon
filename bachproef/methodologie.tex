...
The reasoning, difficulties and setup are documented in Chapter~\ref{ch:poc}.
...
\begin{enumerate}
  \item \textbf{Topology 1 (direct link)}: during this test only two nodes were used, to test the direct connection between the base
...
Although the latency was listed as a metric in the original research proposal (Appendix~\ref{ch:onderzoeksvoorstel}), it was deliberately dropped from the measurement scope.
Neither the Range Test module nor the traceroute function logs round-trip timing, and for the central research question, whether a reliable link exists at all, range and packet delivery proved to be the decisive factors.
The latency has just been observed informally, visually: the traceroute replies and text messages arrived within a few seconds (usually less than 10 seconds), this is good enough for this kind of application.
All those measurements were taken across multiple testing days, different locations, times of day and weather conditions where possible (even though these July/August months in Belgium were extremely dry so no real weather difference could be measured).
...
Using the evaluation criteria established during the first phase a comparison matrix will be built.
