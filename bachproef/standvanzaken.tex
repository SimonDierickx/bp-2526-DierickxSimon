...
One of the subquestions was the secuirty of Meshtastic in case of traffic interception. Thats why Meshtastics encryption model deserves a dedicated analysis.

Meshtastic encrypt the payload of each of its channels using \gls{aes} in \gls{ctr} mode, they have a disticnt key per channel \autocite{MeshtasticEncryption}. One of the curcial decision of Meshtastic is to only encrypt the payload, which means the packet header (containing send node number, packet identifier and hop limit) is not encrypted and always transmitted in cleartext \autocite{MeshtasticEncryption}.
...
This means confidentialityu of the message content and participation in routing are independent: a third-party node can carry traffic without reading it.
...
This means any device worldwide could decrypt traffic on this defautl channel, so for that case no real secuirty or confidentiality is available until a private key is configured.
...
Some recent firmware updates to Meshtastioc (version 2.5 and later) add public-key cryptography but only to direct messages. These provide authentication for one-to-one communication. For group channels however, the shared-key model stays in place and riskful.
...
The range and reliability of Meshtastic is left open on puprose, as this is what needs to be investigated using the \gls{poc} in chapter~\ref{ch:poc}.
