...
This thesis started by investigating o what extent does Meshtastic, as a decentralized \gls{lora}-based mesh network, offer a reliable and readily deployable emergency communication solution when primary network infrastructure fails.
Based on the initial literature study and the \gls{poc}, the answer to this question would be yes, on certain conditions.

The \gls{poc} shoewd that a low-cost Meshtastic setup can provide a real, working link over real distances without relying on any existing infrastructure. A direct connection between two nodes reached a maximum of 18.5\,km, and adding a single relay in this setup improved the end-to-end distance (from Base-Node to mobile node) to an impressive 30\,km, this is larger than the literature study suggested.
The \gls{poc} also proved a relay could convert a dead zone connection into a working connection (session 1 compared to session 3). When comparing against the five inital requirements the literature study gave us in chapter~\ref{ch:literatuurstudie}, Meshtastic can be seen as a realistic alternative.
It performs well on infrastructure independence, deployability and affordability, and for security, the last one is not perfect, but it offers a good level if correctly configured.

For the problem domain, the identified gaps were solutions that don't depend on main infrastructure, which are quick to deploy and are affordable at scale. Both APRS and DMR fail because of the licensing requirements. Satelitte messnegers like Garmin inReach and Zoleo mainly fail because fo the recurring costs.
...
This was clearly illustrated in section~\ref{sec:poc-session2} where the highest test locations (Hotond, Kwaremont) had 0\,\% \gls{pdr} because of the trees, while lower altitrude locations bvut with a clear \gls{los} achieved 100\,\%.
...
If someone gets the key, they can impersonate a node and send messages on the channel. The header of the message is also in cleartext, so the metadata is not protected. The private communication from end-to-end is protected with authentication as well.

The range an reliability question from the solution domain has also been answered in the \gls{poc}.
...
On the security question, just as mentioned before, confidentiality holds up, but authenticaton and metadata protection do not. This has also been discussed in section~\ref{sec:poc-security}.
...
This is important for a real-world deployment: picking a relay location isn't just "fidn the highest point", but it's "finding the point with the best \gls{los} in the direction you need coverage".
...
Some limitations where also discovered. The only tested hardware was the Heltec LoRa32 V4; APRS, DMR, satellite messengers were only evaluated through literature study. Also other ESP32 options exist but weren't tested. They should work on the same way this \gls{poc} was done.
...
Latency wasn't really analysed in depth in the results, the traceroute and messages arrived in a few seconds, but the range and packet delivery turned out to be the more decisive factors for answering the research question.
...
Each node probably add some packet loss, it would be useful to know exactly how much and how far (in terms of hops and distance) this type of network can be pushed before it stops being useful.

Another next step could be testing in different environmental conditions. This was supposed to be done during the \gls{poc}, but except different days and time of the day the weather has been dry and calm from start to finish of the \gls{poc} (20th of July 2026 - 20th of August 2026).
...
Going more into the security side of Meshtastic could be another directin to dig into, especially authentication and metadata protection (section~\ref{sec:sota-security}).
Lastly, as mentionned in section~\ref{sec:poc-session3}, using higher-gain or directional antennas (such as Yagi, parabolic, etc.) instead of a small omnidirectional one could largely improve the quality and the range of the network. The broadcast aspect will most definitely be lost, but in return the range and quality could be extended.
This wasn't tested during this thesis due to cost and time constraints, but it is most definitely a realistic next step for anyone trying to push the research even further.
...
With these points taken into account, Meshtastic stays a genuinely usable, low-cost option for emergency communication, well within the reach of volunteer organizations or small teams without a big budget or specialized licences.