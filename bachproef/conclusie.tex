...
If someone gets the key, they can impersonate a node and send messages on the channel. The header of the message is also in cleartext, so the metadata is not protected. The private communication in between devices (end-to-end, no channel usage) is not only protected with the encryption but this also has authentication compared to the channels.
