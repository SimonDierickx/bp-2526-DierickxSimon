...
This chapter presents and analyses the proof of concept.
...
The next part is presenting and analysing the results of three different field-test sessions: direct-link baseline (Section~\ref{sec:poc-session1}), relay survey (Section~\ref{sec:poc-session2}) and full deployment (Section~\ref{sec:poc-session3}).
...
The main power source was a 6\,W 5\,V/1.2\,A solar panel, the size of this panel ($27.1 \times 17.5 \times 0.26$\,cm) allows it to be easily mounted and directed to the sun.
...
This was on the chosen websites and at the moment of building the setup in Belgium, prices may vary according to time and location. 
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrr}
    ...
    6\,W USB solar panel (5\,V/1.2\,A)            & \euro{}19.99 & 2 & \euro{}39.98 \\
...
The junction box and antenna are oriented towards the base node. From the relay's position the base node is directly visible, as shown in figure~\ref{fig:poc-panoramique} (the red marker indicates the base node).
...
The computer needed the installation of the CH343 driver to be able to flash and configure the board. The Heltec device should also be put into bootloader mode by holding the PRG button while plugging it in.
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    \toprule
    \textbf{Setting} & \textbf{Base-Node (Ronse)} & \textbf{Relay-Node (Mont-Saint-Aubert)} & \textbf{Mobile Node} \\
    \midrule
...
\begin{minted}{batch}
  ...
  %MESH% --setlat 50.72316 --setlon 3.63217 --setalt 90
  timeout /t 5 /nobreak > nul
...
In the configuration file the \texttt{lora.tx\_power} is set to \texttt{0}. This doesn't mean that the power is 0\,dBm, but the maximum the regional settings allow.
For the EU\_868 band this means 27\,dBm, that's why table~\ref{tab:poc-config} and appendix~\ref{ch:bijlage-config} report 27\,dBm.
...
Taken together, these three segments illustrate that Session~1's \gls{pdr} was highly location-dependent rather than a single figure: 78.6\,\% over the first three kilometres, 0\,\% in the intermediate dead zone, and 68.8\,\% during the final clear-line-of-sight climb.
The 99.7\,\% reported later (Figure~\ref{fig:poc-snr-stab}) refers specifically to the stationary maximum-range link once a clear line-of-sight was established, not to the moving drive.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{plot-snr-histogram.jpg}
  \caption[SNR distribution at 18.5\,km.]{\label{fig:poc-snr-hist}Distribution of the \gls{snr} values of the 1\,015 packets received during the stationary test (18.5\,km at Mont-Saint-Aubert). 91.7\,\% of packets had a positive \gls{snr}, with a median of $+6$\,dB, confirming the maximum-range link was of consistently high quality.}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{plot-snr-stability.jpg}
  \caption[Link stability at 18.5\,km.]{\label{fig:poc-snr-stab}The same 1\,015 packets from the stationary test, plotted against their sequence number (from 1 to 342, 341 received: 99.7\,\% \gls{pdr}). After some initial variation, the link stabilised around $+6$\,dB, showing the maximum-range link was sustainable.}
\end{figure}
...
At each location the \gls{rssi}, \gls{snr} and \gls{pdr} on the direct link to the base node were recorded, together with the altitude and an assessment of the \gls{los}. Each chosen spot was marked using its coordinates for precise reporting.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{plot-los-altitude.jpg}
...
As shown in figure~\ref{fig:poc-los}, some sites had good \gls{pdr} and \gls{rssi}, some had good altitude, but none of them combined both successfully.
...
The confirmed location of the relay node was then 50.65457, 3.39975 at an altitude of 140\,m.
...
In this situation an ideal setup would have been a hill on the opposite side of the city from where the relay is, which would have given more coverage in the city center, and a connection further away, in the flat plains of Wallonia. 
...
This is the self-organising nature of Meshtastic. Nodes discover one another and exchange necessary information for routing. This makes the range instantly larger than what you originally expected.
...
The difference is that now the third-party nodes can forward the messages but they can't read them.
...
\subsection{Opportunistic wet-weather check}
\label{sec:poc-rain}

Throughout the whole testing period the weather in Belgium (Flemish Ardennes and Pays des Collines) was very dry. This made testing weather variations impossible to measure during the sessions.
However at the end of the period, on 17 August 2026 at 06:02, a single traceroute could be performed during heavy rain. This traceroute was done between the Ronse base node and the relay node at Mont-Saint-Aubert.
The traceroute returned a successful result. The Ronse\,$\rightarrow$\,relay link had an \gls{rssi} of $-114$\,dBm and an \gls{snr} of $-10.75$\,dB. The relay\,$\rightarrow$\,Ronse link had an \gls{rssi} of $-114$\,dBm and an \gls{snr} of $-4.75$\,dB.
This single observation cannot claim the connection is weather variation proof, but it does show that in this occasion the link was still operational.
What can be seen is that, although the link was successful, the quality of the connection was degraded, so this should be further investigated.