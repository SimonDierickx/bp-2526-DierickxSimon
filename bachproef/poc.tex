...
At the end of this chapter we can also observe other nodes, not known to us participating in the mesh network (Section~\ref{sec:poc-mesh}), the security demonstration (Section~\ref{sec:poc-security}) and the evauilations of autonomou solar working (Section~\ref{sec:poc-solar}).

\section{Hardware selection}%
\label{sec:poc-hardware}

The main constraint while choosing hardware was cost (this is one the requirements for emergencies).
...
The final hardware choice was to use the \textbf{Heltec WiFi LoRa 32 V4}. This is a board using an Espressif ESP32-S3 microcontroller and the Semtech SX1262 LoRa tranceiver.
...
The V4 was chosen over the V3 (cheaper model) because it provides solar-charging input, higher maximum transit power and the possbility to add a battery.
These features are important in this use case. 
The SX1262 tranceiver provides a better sensitivity compared to the SX1276 (used on older models), this is important for long-range usage.

Another important piece of hardware are the antennas. The provided antenna with the board is a small PCB antenna which is unsable in long-range scenarios and if the board in placed in an enclosure.
Having to place the nodes outside this would also mean the antenna is unusable. That's why for the two fixed ndoes (base and relay) a 5\,dBi external antenna was chosen. This antenna is omnidirectional which allows us to have a good covergae, despite the orientation.
The main V4 boards where then mounted inside the enclosure and the external antenna was connected using a u.FL-to-SMA cable.
The mobile node was equipped with a 868\,MHz antenna with a magnetic base so that it could be mounted on the car roof.

For realistic emergency situations botht the fixed nodes were not plugged in a regular electrical socket. That's why for both the fixed nodes a small 2000mAh/3.7V LiPo battery was used as a buffer.
The main power sources was a 6W 5V/1A solar panel, the size of this panel (27,1 x 17,5 x 0,26 cm) allows it to be easily mounted and directed to the sun.
This way the nodes uses the solar panel to be powered on and charge the battery during the day, at night or at cloudy weather the battery takes over.
On the base node the solar panel was soldered to a JST\,1.25mm connector so the USB-C plug could be used for data transfer.
...
This is a relevant result in itself for the scope of this thesis. The complete setup (four ndoes, including a spare node, cables, antennas, batteries, solar panels and enclosures) was acquired for \euro{}185 in total.
This was on the chosen sites and at moment of building the setup in Belgium, prices may vary according to time and location. 

The bought components were not neecssarily the cheapest available at the moment on the market, but due to the time constraints this project fell in, some products had to be chosen from more expensive EU-based resellers instead of non-EU resellers, which may be a lot cheaper, in order to achieve it in time.
But even given this, the total cost of the setup compared ti the cost of a commercial solution (e.g. a single Garmin inreach device) costs more than the fulls etup before even paying any subscription or fee.
The figures below represent a realistic upped bound on the cost, this should not be take as a cost-optimized setup. According to estimations, a total of 100/150€ could be done, according to the amount of nodes needed, etc.
...
\begin{table}
  ...
  \begin{tabular}{lrrr}
    ...
    LiPo battery 2000\,mAh (JST 1.25\,mm)       & \euro{}9.64  & 2 & \euro{}19.28 \\
...
The fixed nodes were assembled inside an IP66 junction box. The V4 board was splaced with dual-sided tape inside the box, together with the LiPo battery (JST 1.25mm connector).
...
The complete setup can be seen in figure~\ref{fig:poc-build-ronse.jpg} down below.
The relay node was installed inside the viewing tower of Floreal Le Panoramique on the Mont-Saint-Aubert (50.65457,
3.39975) with an altitude of 140\,m. Permission was obtained by the owners. The installation as seen in figure~\ref{fig:poc-build-relay.jpg} shows the solar panel is set up to be facing southwards.
...
\begin{figure}
  ...
  \caption[Relay installation.]{\label{fig:poc-build-relay}The relay node installation inside the viewing tower of Floreal Le Panoramique. The red marker indicates the position of the base node.}
\end{figure}
...
A phone was connected through Bluetooth to the mobile node, allowing it to broadcast it's real-time GPS location.

\section{Configuration and setup difficulties}%
\label{sec:poc-config}

Using the \href{https://flasher.meshtastic.org/}{Meshtastic firmware flasher} the last stable version of Meshtastic (2.7.26.54e0d8d) was flashed on all four nodes.
...
On all three nodes the radio profile was set on the EU\_868 region, LONG\_FAST modem preset (which is 250\,kHz bandwidth, spreading factor 11, coding rate 4/5), a hop limit of 3, a transmit power of 27\, dBm (as allowed in the EU\_868 band),
boosted receiver gain enabled and the FEM/LNA mode disabled (the V4 has no external low-noise amplifier).
However the three nodes received three different roles: base node in Ronse as \texttt{CLIENT\_BASE} (sending, receiving and forwarding messages), relay node as \texttt{ROUTER} (forwarding messages) and mobile node as \texttt{CLIENT} (receving and sending messages). 
The Meshtastic Range Test module was enabled and configured so that the mobile node was sending a test packet each 60 seconds to the base node. The base node then kept this in a CSV log file for later analysis.
These sequences contained a sequence number, time of reception, RSSI, PDR and the exact position of the node (this devices doesn't have a GPS module, but it uses the phones shared GPS position).
...
\begin{itemize}
    \item The web-based configurator was unreliable for bulk configuring the device. Settings appeared as saved but fresuently weren't really saved after the reboot. The Python CLI and mobile application were both more reliable, so thats what was used.
...
The first test session consisted of discovering the usage of Meshtastic and LoRa technology, as well as testing a direct link between the Ronse base node and the mobile node.
...
The PDR (Packet Delivery Rate) was 78.6\,\% and the RSSI degraded from $-51$\,dBm to $-119$\,dBm as the distance increased.
After these initial three kilometres a first complete dead zone was found, lower altitude, no line-of-sight with the home node, these factors made communication impossible, 0\,\% PDR.
...
While ascending the Mont-Saint-Aubert the total PDR of the climb was 68.8\,\% and the RSSI stabilized around $-107$\,dBm with an SNR of approximately $+1$\,dB.
The maximum distance at which communiction was elaborated that day was 18.5\,km.

The key takeaway from this first testing session was that what the distance isn't the real challenge in our scenario. The terrain obstruction and abscence of line-of-sight is the main limiting factor for long-range communication with these types of devices.
The dead zone was at 8--15\,km while we had a link at 18.5\,km.
The measured RSSI $-107$\,dBm also sits within the SX1262 sensitivity of approximately $-132$\,dBm at the LONG\_FAST setting.
This means that there was a $25$\,dBm margin.
...
The locations checked where around the Hotond, Kwaremont, Trieu and Kanarieberg. These locations are all elevated and have a somewhat clear line-of-sight with the base node.
At each location the RSSI, SNR and packet delivery rate on the direct link to the base node were recorded, together with the altitude and an assessment of the line of sight. Each chosen spot was marked using it's coordinates for precise reporting.
...
Lower locations that possessed a clear LOS like Karnemelkbeekstraat (80\,m) and Drie Lindenstraat (87\,m) achieved 100\,\% packet delivery at a distance of 7.5--7.6\,km.

This confirms our initial finding of session 1, LOS is more important than elevation. Elevation is really beneficial, but only when combined with a clear LOS.
...
As seen some sites had good \gls{pdr} and \gls{rssi}, some had good altitude, but none of them combined both.
The best possiblities from this session where the Malanderpark and Zandstraat tests. But these sites only had a straight line distance of 5.1--5.7\,km to the base node.
On top of this, they didn't have a straight forward mounting point (houses, trees, poles, etc.) so an extra requirement would have been to ask permission to the relevant persons, which was complicated in this situation.

The Mont-Saint-Aubert possiblity stands out as the best option, combining elevation, clear \gls{los} and a person working there who could help with an accessible mounting point.
...
to the base node passing through the relay node. The connection between the mobile and relay node had a connection with an RSSI between $-90$ and $-106$\,dBm. The distance between this location and the relay node was only 4.7km but no clear \gls{los} was present, the relay node was 140\,m high and the mobile node was at 30\,m.
The distance between the relay and base node was 18.5\,km, which means the total distance travelled by the mesage was 23.2\,km, straight line distance between the base and mobile node was 19.71\,km.

During this test session the maximum end-to-end distance reached was 30km, with a working connection at 11.5\,km from the relay node.
...
This relay also revealed the effective limitations of Meshtastic using this relay configuration. Locations at low altitude (under 30\,m) at more then 10\,km from the relay where failing to connect. Locations in the city center of Tournai, which is densely built up, also failed.
...
These findings confirm what was found in the first two test sessions. The obtruction dominates over the altitude, too much obstacles and the connection is impossible or marginal.
In this situation a perfectr setup would have been a hill on the opposite side of the city from where the relay is, which would have given more coverage in the city center, and a connection further away, in the flat plains of Wallonia. 
...
The effective coverage could be imporved and extended by placing nodes in hgiher and less obstructed locations. Replacing the used antennas with higher end (higher gain, directional) antennas such as Yagi or parabolic antennas on the fixed nodes. These anteznnas would concentrate on a specific axis, which would improve the link but limit the broadcast.
This wasn't the main goal in this thesis as the cost effectiveness was also investigated and we aimed for a wider coverage, this could be changed in fucntion of the goals.
They are, if relevant, a good direction to improve network quality and extend the actual coverage. These are revisted in Chapter~\ref{ch:conclusie}.
...
Other devices (Heltec, LilyGo, etc.) configured witht he Meshtastic firmware communicated with the deployed nodes, and actively participated in some connections: forwarding messages from mobile to base when the relay was not deployed yet, etc.
...
This shows the mesh is a real, practical solution for estazblishing a connection between distant points. They however could forward the packages, but they couldn't read any content of these packages.
...
During the test sessions third-party notes appeared (as dicussed in section \ref{sec:poc-mesh}). At that point our nodes were still running the
default \gls{psk} beiing \texttt{AQ==}. This means that each node intercepting and forwarding the traffic could decrypt the messages. This is of course not safe, so the \gls{psk} was changed to a random 256-bit key.
...
Some parts of the packages are not encrypted, and these parts guide the routing of that package (node identifiers, timing, hop count).
...
Another limitation found is that the \gls{psk} is the same on all three nodes, which means if one of the ndoes get "hacked" none of the traffic still is secure, also the fact the \gls{psk} is the same
on each node it means the key has to be shared to each participant node, which also involves a security issue.

\section{Autonomous solar operation}%
\label{sec:poc-solar}

In the end, the autonomous operation of said notes was also evaluated. These nodes having to be deployed during an emergency, we can't ensure the main electrical supply survived.
To avoid any of these issues the test was conducted using a LiPo battery (3.7\,V, 2000\,mAh) and a 6W solar panel (for charging the battery and operation during day time).
This setup was used for both the fixed nodes.

The relay node was set up as "Power saving" mode, while the base ndoe was completely set up without any limits, and even WiFi enabled (which consumes even more pwower).
Theoretically, the battery of 2000\,mAh and 3.7\,V (storing 7.4\,Wh) shoudl be bale to run the setup during 20 to 30 hours. This is calculated using an estimation of 0.3\,W of usage on the setup.
